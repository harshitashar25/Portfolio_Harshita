  ...
  const responsibilities = [
    {
      ...
      icon: <Users className="text-[#ADEBFF]" size={24} />
    },
    {
      ...
      icon: <Briefcase className="text-[#ADEBFF]" size={24} />
    }
  ];

  return (
    <section id="responsibility" className="py-20 bg-[#00171F] text-white">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-bold mb-2 text-center text-[#ADEBFF]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {responsibilities.map((position, index) => (
            <Card key={index} className="animate-fade-in bg-[#00171F] border border-[#ADEBFF]/20" style={{ animationDelay: `${index * 0.1}s` }}>
              <CardContent className="p-6">
                <div className="flex items-start gap-4">
                  <div className="mt-1 p-2 bg-[#ADEBFF]/10 rounded-full">
                    {position.icon}
                  </div>
                  <div>
                    <h3 className="text-xl font-semibold text-[#ADEBFF]">{position.title}</h3>
                    <div className="flex justify-between items-center mt-1 mb-3">
                      <p className="text-[#C2E6FF]">{position.organization}</p>
                      <span className="px-2 py-1 bg-[#002B3A] text-[#ADEBFF] text-xs rounded">
                        {position.duration}
                      </span>
                    </div>
                    <ul className="list-disc pl-5 space-y-1 text-[#C2E6FF]">
